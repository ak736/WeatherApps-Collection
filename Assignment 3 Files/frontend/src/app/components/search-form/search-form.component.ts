...
export class SearchFormComponent implements OnInit {
  ...
  private initForm(): void {
    ...
    this.searchForm.get('useCurrentLocation')?.valueChanges.subscribe(checked => {
  ...
  onSubmit(): void {
    if (this.searchForm.invalid && !this.searchForm.get('useCurrentLocation')?.value) {
      return;
    }
...
fetchWeatherUsingCurrentLocation(): void {
    ...
    this.http.get<WeatherResponse>('http://localhost:3000/api/location-weather')
      .subscribe({
        next: (response) => {
        ...
        error: (error) => {
          ...
          this.error = 'Failed to fetch weather data: ' + (error.message || 'Unknown error');
          this.isLoading = false;
        }
      });
  }
  ...
  fetchWeatherUsingAddress(): void {
    ...
    this.http.get<WeatherResponse>('http://localhost:3000/api/address-weather', { params })
      .subscribe({
        next: (response) => {
          const filteredIntervals = this.filterCurrentAndFutureDates(
            response.forecast.data.timelines[0].intervals
