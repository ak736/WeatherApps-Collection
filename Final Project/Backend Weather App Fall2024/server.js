...
app.get('/api/places/autocomplete', async (req, res) => {
  try {
    ...
    // Enhanced filtering of predictions
    const predictions = response.data.predictions
      .filter((prediction) => {
        // Only include results where the first term is the city
        const terms = prediction.terms
        return (
          terms.length >= 2 &&
          !terms[0].value.includes('AFB') && // Filter out Air Force Bases
          !terms[0].value.includes('Base')
        ) // Filter out other military bases
      })
      .map((prediction) => ({
        city: prediction.terms[0].value,
        state: prediction.terms[1].value,
        description: prediction.terms[0].value,
      }))
...
app.get('/api/weatherSearch', async (req, res) => {
  try {
    ...
    // First get the city/state using Google Geocoding API
    const geocodeResponse = await axios.get(
      'https://maps.googleapis.com/maps/api/geocode/json',
      {
        params: {
